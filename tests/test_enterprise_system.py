...
def test_invalid_input_handling():
    ...
    # جرب أنواع مختلفة من المدخلات غير الصالحة
    invalid_inputs = [
        "invalid_string",
        ["list", "input"], 
        {"dict": "input"},
    ...
    for invalid_input in invalid_inputs:
        try:
            result = sentinel.analyze_model_enterprise(invalid_input, "bad_model")
            # إذا وصلنا هنا، الدالة تعاملت مع المدخلات غير الصالحة
            assert "health_score" in result  # تحقق أن هناك نتيجة
        except (ValueError, TypeError) as e:
            # هذا أيضاً مقبول - رفع استثناء
            print(f"✅ الدالة رفعت استثناء: {type(e).__name__}")
...
def test_b2_file_upload_download():
    ...
    # Download and verify - تحقق من أن العملية ناجحة دون مقارنة المحتوى
